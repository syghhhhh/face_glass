...
def merge_audio(afile, vfile, outfile):
    ...
    currentPwd = ''
    command = "ffmpeg -i {} -i {} -c:v copy -c:a aac -strict experimental -crf 0 {} -loglevel quiet".format(currentPwd + vfile,
                                                                                            currentPwd + afile,
                                                                                            currentPwd + outfile)
...
# parser.add_argument("--glass", default=os.path.join(base_path, "glass/20.jpg"), type=str)
# args = parser.parse_args()
#
# path = args.input
# output = args.output
# if not output:
#     output = path[: -len(path.split('.')[-1]) - 1] + '_out.' + path.split('.')[-1]


def run_ori(path, output, glass):
